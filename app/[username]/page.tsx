...
// Social Icon Mapping (Matches Dashboard)
const iconMap: any = {
...
export default function PublicProfile({ params }: { params: { username: string } }) {
  ...
  // Filter out links that have no URL
  ...
  return (
    <div className="container">
      <style jsx>{`
        ...
        .pfp {
          width: 95px; height: 95px; object-fit: cover; margin-bottom: 16px;
          border-radius: 50%;
        ...
        .badges-pill {
          ...
          padding: 4px 8px; border-radius: 10px; border: 1px solid rgba(255, 255, 255, 0.05);
        }
        ...
        .tags-row { 
          ...
          gap: 10px; margin-bottom: 18px; opacity: 0.6;
        }
          ...
          {(profile.badges?.user || profile.badges?.dev || profile.badges?.staff) && (
            <div className="badge-anchor">
              <div className="badges-pill">
                {profile.badges?.user && <ShieldCheck size={16} color="rgba(255,255,255,0.6)" />}
                {profile.badges?.dev && <Code size={16} color={accent} />}
                {profile.badges?.staff && <Star size={16} color="rgba(255,255,255,0.6)" />}
              </div>
            </div>
          )}
        </div>
